...
export default function ExamPage() {
  ...
  const initializeExam = async () => {
    try {
      ...
      // 2. ดึง Session (ใช้ session_id จาก URL หรือหา session ล่าสุด)
      let sessionData = null
      const sessionIdFromUrl = searchParams.get('session_id')
      
      if (sessionIdFromUrl) {
        console.log("📋 Fetching session by ID:", sessionIdFromUrl)
      ...
      if (!sessionData) {
        ...
        alert("ไม่พบตารางสอบ - กรุณาเริ่มสอบจากหน้า Lobby")
        router.push("/student/lobby")
        return
      }

      console.log("✅ Session found:", sessionData.session_name)
      ...
      if (now < start) {
        alert("การสอบยังไม่เริ่ม")
        router.push("/student/lobby")
        return
      }

      if (now > end) {
        alert("การสอบสิ้นสุดเวลาแล้ว")
        router.push("/student/lobby")
        return
      }

      // 4. สร้างหรืออัปเดต Exam Record (ใช้ upsert)
      console.log("📝 Creating/updating exam record...")
      ...
      if (recordResult.error) {
        console.error("❌ Record error:", recordResult.error)
        throw recordResult.error
      }

      const currentRecordId = recordResult.data?.id
      setRecordId(currentRecordId)
      console.log("✅ Record ID:", currentRecordId, "Status:", recordResult.data?.status)
      ...
      if (recordResult.data?.status === 'ongoing') {
        ...
        if (answersResult.data && answersResult.data.length > 0) {
          ...
          answersResult.data.forEach((a: any) => {
            answerMap[a.question_id] = a.selected_choice_id
          })
          setAnswers(answerMap)
          console.log(` Resumed ${answersResult.data.length} answers`)
        }
      }
      ...
      console.log(`⏱️ Time remaining: ${remaining}ms`)
    ...
    try {
      ...
      for (const q of shuffledQuestions) {
        ...
        if (choicesResult.data && choicesResult.data.length > 0) {
          const correctChoices = choicesResult.data.filter((c: Choice) => c.is_correct)
          const wrongChoices = choicesResult.data.filter((c: Choice) => !c.is_correct)
          const shuffledWrong = wrongChoices.sort(() => Math.random() - 0.5).slice(0, 3)
          const mixedChoices = [...correctChoices, ...shuffledWrong].sort(() => Math.random() - 0.5)
          
          finalQuestions.push({ ...q, choices: mixedChoices })
        }
      }
  ...
  const submitExam = async () => {
    ...
    if (timeLeft > 0 && !confirm("คุณแน่ใจหรือไม่ว่าต้องการส่งข้อสอบ?")) {
      return
    }
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <p className="text-lg text-red-600">ไม่พบข้อสอบ</p>
      </div>
    )
  }
        ...
        <div className="max-w-4xl mx-auto px-4 py-3">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className={`text-2xl font-mono font-bold px-4 py-1 rounded-lg ${
              timeLeft < 300000 ? "bg-red-100 text-red-600 animate-pulse" : "bg-teal-100 text-teal-700"
            }`}>
      ...
      <div className="max-w-4xl mx-auto mt-6 px-4">
                ...
                return (
                  <label 
                    ...
                    className={`flex items-start p-4 rounded-xl border-2 cursor-pointer transition-all ${
                      isSelected ? "border-teal-500 bg-teal-50" : "border-gray-200 hover:bg-gray-50"
                    }`}
                  >
                    <input 
                      type="radio" 
                    ...
                    <div className="ml-3 flex-1">
                      <span className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-gray-200 text-gray-700 font-semibold text-sm mr-3">
                        {letters[index]}
                      </span>
        ...
        <div className="flex justify-between items-center mt-6 gap-4">
          <button 
            ...
            className="px-6 py-3 bg-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-300 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            ← ข้อก่อนหน้า
          </button>
          ...
          {currentIndex === questions.length - 1 ? (
            <button 
              onClick={submitExam} 
              disabled={submitting} 
              className="px-8 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-bold hover:from-green-600 hover:to-emerald-700 shadow-lg disabled:opacity-50"
            >
            ...
            <button 
              onClick={() => goToQuestion(currentIndex + 1)} 
              className="px-6 py-3 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-xl font-semibold hover:from-teal-600 hover:to-cyan-700 shadow-lg"
            >
              ข้อถัดไป →
            </button>
          )}
